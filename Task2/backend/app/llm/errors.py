...
"Something went wrong" is never a useful thing to show someone. An expired key, an
exhausted quota and a model the account cannot access all need different actions,
and the API tells us which one it is -- so say so. Wording adapts to the configured
provider, since the fix lives in a different console for each.
"""
...
def describe_llm_error(exc: Exception) -> str | None:
    ...
    if isinstance(exc, openai.RateLimitError):
        ...
        if _is_quota_exhausted(exc):
            ...
            return f"The {name} quota is used up, so this could not be processed.{soon}{where}"
        return (
            f"{name} is rate limiting requests right now."
            f"{soon or ' Wait a few seconds and try again.'}"
        )
    ...
    if isinstance(exc, openai.PermissionDeniedError):
        return (
            ...
            f"'{settings.condense_model}'. Set LLM_CHAT_MODEL and LLM_CONDENSE_MODEL in "
            "backend/.env to models this key can use."
        )

    if isinstance(exc, openai.NotFoundError):
        return (
            f"{name} does not recognise the configured model. Set LLM_CHAT_MODEL / "
            "LLM_CONDENSE_MODEL in backend/.env to a model this key can use."
        )
