...
74 tests. They need **no API key and no network** — the link checker is driven through an
...
Four guards, because this is the feature most likely to lose someone's afternoon:
...
### A finished plan is never regenerated by accident

Routing decides what a turn does, and `_resolve_action` promotes "ask" to "generate" once the intake
has nothing left to ask about. That is correct on the first pass and destructive on the tenth: the
read step only has to misjudge one conversational message for the whole course, and every hand edit
in it, to be rebuilt from scratch — and `generate` has no equivalent of the refinement guard to
catch it. So the promotion only happens when there is no plan yet. With a plan on the page the turn
answers the question and leaves the plan alone.

Starting over on purpose is still allowed, and it carries the version forward rather than restarting
the count, for the same reason the precondition exists: the browser is holding version N, and
dropping back to 1 would let an edit already queued there pass the check and land on top of the plan
